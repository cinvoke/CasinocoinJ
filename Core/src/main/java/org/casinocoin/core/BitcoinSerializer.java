 ...
 * <a href="https://en.casinocoin.it/wiki/Protocol_specification">the protocol specification</a>.</p>
 *
...
public class BitcoinSerializer {
    ...
    public void serialize(String name, byte[] message, OutputStream out) throws IOException {
        ...
        byte[] hash = Utils.doubleDigest(message);
        System.arraycopy(hash, 0, header, 4 + COMMAND_LEN + 4, 4);
        out.write(header);
        out.write(message);

        if (log.isDebugEnabled())
            log.debug("Sending {} message: {}", name, Utils.bytesToHexString(header) + Utils.bytesToHexString(message));
    }
    ...
    public Message deserializePayload(BitcoinPacketHeader header, ByteBuffer in) throws ProtocolException, BufferUnderflowException {
        ...
        // Verify the checksum.
        byte[] hash;
        hash = Utils.doubleDigest(payloadBytes);
        if (header.checksum[0] != hash[0] || header.checksum[1] != hash[1] ||
                header.checksum[2] != hash[2] || header.checksum[3] != hash[3]) {
            throw new ProtocolException("Checksum failed to verify, actual " +
                    Utils.bytesToHexString(hash) +
                    " vs " + Utils.bytesToHexString(header.checksum));
        }
