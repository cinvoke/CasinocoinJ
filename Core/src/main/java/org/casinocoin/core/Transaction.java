...
import org.casinocoin.script.ScriptOpCodes;
...
public class Transaction extends ChildMessage implements Serializable {
     ...
     * Currently this is 100000 satoshis.
     */
    public static final BigInteger REFERENCE_DEFAULT_MIN_TX_FEE = CoinDefinition.DEFAULT_MIN_TX_FEE;//= BigInteger.valueOf(1000000);
    ...
    // These are serialized in both casinocoin and java serialization.
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, org.casinocoin.script.Script, ECKey, Transaction.SigHash, boolean)}
    ...
    public synchronized Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        //
        //   https://en.casinocoin.it/wiki/Contracts
        ...
        try {
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                for (int i = 0; i < inputIndex; i++)
                    this.outputs.set(i, new TransactionOutput(params, this, Utils.NEGATIVE_ONE, new byte[] {}));
                // The signature isn't broken by new versions of the transaction issued by other parties.
                for (int i = 0; i < inputs.size(); i++)
                    if (i != inputIndex)
                        inputs.get(i).setSequenceNumber(0);
            ...
            Sha256Hash hash = new Sha256Hash(Utils.doubleDigest(bos.toByteArray()));
            bos.close();
     ...
     * This is useful in certain types of <a href="http://en.casinocoin.it/wiki/Contracts">contracts</a>, such as
