...
import org.casinocoin.params.RegTestParams;
...
public class WalletTool {
    ...
    private static org.casinocoin.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith(CoinDefinition.coinURIScheme)) {
            ...
            try {
                paymentRequest = org.casinocoin.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
