 ...
 * so you can use {@link #calculateBalanceForAddress(org.casinocoin.core.Address)} to quickly look up
 * the quantity of casinocoins controlled by that address.</p>
 */
public class MySQLFullPrunedBlockStore implements FullPrunedBlockStore {
    ...
    private static final String MYSQL_DUPLICATE_KEY_ERROR_CODE = "1022";
    ...
    private static final String CREATE_SETTINGS_TABLE = "CREATE TABLE store_settings (\n" +
    ...
    private static final String CREATE_UNDOABLE_TABLE_INDEX = "ALTER TABLE store_undoableblocks ADD INDEX idx_height (height)";

    // Some indexes to speed up inserts
    private static final String CREATE_HEADERS_HASH_INDEX = "ALTER TABLE store_headers ADD INDEX idx_headershash USING btree (hash);";
    private static final String CREATE_OUTPUTS_ADDRESS_INDEX = "ALTER TABLE store_openoutputs ADD INDEX idx_address USING btree (hash, ooindex, height, toaddress);";
    private static final String CREATE_OUTPUT_ADDRESS_TYPE_INDEX = "ALTER TABLE store_openoutputs ADD INDEX idx_addresstargetable USING btree (addresstargetable);";
    private static final String CREATE_OUTPUTS_HASH_INDEX = "ALTER TABLE store_openoutputs ADD INDEX idx_openoutputshash_1 USING btree (hash);";
    private static final String CREATE_OUTPUTS_HASH_INDEX_INDEX = "ALTER TABLE store_openoutputs ADD INDEX idx_openoutputshash_2 USING btree (hash, ooindex);";
    private static final String CREATE_UNDOABLE_HASH_INDEX = "ALTER TABLE store_undoableblocks ADD INDEX idx_undoableblockshash USING btree (hash);";
     ...
     * @throws org.casinocoin.store.BlockStoreException if the database fails to open for any reason
     */
    public MySQLFullPrunedBlockStore(NetworkParameters params, int fullStoreDepth, String hostname, String dbName,
                                     String username, String password) throws BlockStoreException {
        this(params, "jdbc:mysql://" + hostname + "/" + dbName, fullStoreDepth, username, password);
    }

    /**
     * <p>Create a new MySQLFullPrunedBlockStore, using the full connection URL instead of a hostname and password.</p>
     *
     * <p>The connection URL will be passed to the database driver, and should look like
     * "jdbc:mysql://host[:port]/databasename".  You can use this to change the port, or specify additional parameters.
     * See <a href="http://dev.mysql.com/doc/connector-j/en/connector-j-reference-configuration-properties.html"> of
     * the MySQL JDBC documentation</a> for more on the connection URL.</p>
     *
     * @param params A copy of the NetworkParameters used.
     ...
     * @throws org.casinocoin.store.BlockStoreException If the database fails to open for any reason.
     */
    public MySQLFullPrunedBlockStore(NetworkParameters params, String connectionURL, int fullStoreDepth,
                                     String username, String password) throws BlockStoreException {
        this.params = params;
    ...
    private synchronized void maybeConnect() throws BlockStoreException {
        if(useDatasource){
            try {
                ...
                throw new BlockStoreException(ne);
            }
        } else {
            try {
                if (conn.get() != null)
    ...
    private synchronized void datasourceConnect() throws BlockStoreException, NamingException {
        try {
            if (conn.get() != null)
                return;
    ...
    private void initDatabase() throws BlockStoreException{
        try {
            // Create tables if needed
            if (!tableExists("store_settings"))
                createTables();
            initFromDatabase();
    ...
    public void resetStore() throws BlockStoreException {
        ...
        try {
            ...
            s.execute("DROP TABLE store_settings");
            s.execute("DROP TABLE store_headers");
            s.execute("DROP TABLE store_undoableblocks");
            s.execute("DROP TABLE store_openoutputs");
            s.close();
            createTables();
            initFromDatabase();
        } catch (SQLException ex) {
    ...
    private void createTables() throws SQLException, BlockStoreException {
        ...
        if (log.isDebugEnabled())
            log.debug("MySQLFullPrunedBlockStore : CREATE store_headers table");
        s.executeUpdate(CREATE_HEADERS_TABLE);

        if (log.isDebugEnabled())
            log.debug("MySQLFullPrunedBlockStore : CREATE store_settings table");
        s.executeUpdate(CREATE_SETTINGS_TABLE);

        if (log.isDebugEnabled())
            log.debug("MySQLFullPrunedBlockStore : CREATE store_undoableblock table");
        s.executeUpdate(CREATE_UNDOABLE_TABLE);

        if (log.isDebugEnabled())
            log.debug("MySQLFullPrunedBlockStore : CREATE store_undoableblock index");
        s.executeUpdate(CREATE_UNDOABLE_TABLE_INDEX);
        if (log.isDebugEnabled())
            log.debug("MySQLFullPrunedBlockStore : CREATE store_openoutputs table");
        ...
        s.executeUpdate("INSERT INTO store_settings(name, value) VALUES('" + CHAIN_HEAD_SETTING + "', NULL)");
        s.executeUpdate("INSERT INTO store_settings(name, value) VALUES('" + VERIFIED_CHAIN_HEAD_SETTING + "', NULL)");
        s.executeUpdate("INSERT INTO store_settings(name, value) VALUES('" + VERSION_SETTING + "', 0x03)");
        s.close();
    ...
    private void initFromDatabase() throws SQLException, BlockStoreException {
        ...
        rs = s.executeQuery("SELECT value FROM store_settings WHERE name = '" + CHAIN_HEAD_SETTING + "'");
        if (!rs.next()) {
        ...
        rs = s.executeQuery("SELECT value FROM store_settings WHERE name = '" + VERIFIED_CHAIN_HEAD_SETTING + "'");
        if (!rs.next()) {
    ...
    public void dumpSizes() throws SQLException, BlockStoreException {
        ...
        long totalSize = 0;
        int count = 0;
        ResultSet rs = s.executeQuery("SELECT name, value FROM store_settings");
        while (rs.next()) {
        ...
        rs = s.executeQuery("SELECT chainWork, header FROM store_headers");
        while (rs.next()) {
            size += 28; // hash
        ...
        totalSize += size; size = 0; count = 0;
        rs = s.executeQuery("SELECT txOutChanges, transactions FROM store_undoableblocks");
        while (rs.next()) {
            size += 28; // hash
            size += 4; // height
        ...
        totalSize += size; size = 0; count = 0;
        long scriptSize = 0;
        rs = s.executeQuery("SELECT value, scriptBytes FROM store_openoutputs");
        while (rs.next()) {
            size += 32; // hash
            size += 4; // index
            size += 4; // height
    ...
    private void putUpdateStoredBlock(StoredBlock storedBlock, boolean wasUndoable) throws SQLException {
        try {
            PreparedStatement s =
                    conn.get().prepareStatement("INSERT INTO store_headers(hash, chainWork, height, header, wasUndoable)"
                            + " VALUES(?, ?, ?, ?, ?)");
            ...
            if  (!(e.getSQLState().equals(MYSQL_DUPLICATE_KEY_ERROR_CODE)) || !wasUndoable)
                throw e;

            PreparedStatement s = conn.get().prepareStatement("UPDATE store_headers SET wasUndoable=? WHERE hash=?");
            s.setBoolean(1, true);
            // We skip the first 4 bytes because (on prodnet) the minimum target has 4 0-bytes
            byte[] hashBytes = new byte[28];
            System.arraycopy(storedBlock.getHeader().getHash().getBytes(), 3, hashBytes, 0, 28);
    ...
    public void put(StoredBlock storedBlock, StoredUndoableBlock undoableBlock) throws BlockStoreException {
        ...
        try {
            ...
            PreparedStatement findS = conn.get().prepareStatement("select 1 from store_undoableblocks where hash = ?");
            ...
            if (rs.next())
            {
                ...
                PreparedStatement s =
                        conn.get().prepareStatement("UPDATE store_undoableblocks SET txOutChanges=?, transactions=?"
                                + " WHERE hash = ?");
            ...
            PreparedStatement s =
                    conn.get().prepareStatement("INSERT INTO store_undoableblocks(hash, height, txOutChanges, transactions)"
                            + " VALUES(?, ?, ?, ?)");
            s.setBytes(1, hashBytes);
            s.setInt(2, height);
            ...
            if (!e.getSQLState().equals(MYSQL_DUPLICATE_KEY_ERROR_CODE))
                throw new BlockStoreException(e);
        }
    ...
    public StoredBlock get(Sha256Hash hash, boolean wasUndoableOnly) throws BlockStoreException {
        ...
        try {
            s = conn.get()
                    .prepareStatement("SELECT chainWork, height, header, wasUndoable FROM store_headers WHERE hash = ?");
            // We skip the first 4 bytes because (on prodnet) the minimum target has 4 0-bytes
            byte[] hashBytes = new byte[28];
    ...
    public StoredUndoableBlock getUndoBlock(Sha256Hash hash) throws BlockStoreException {
        ...
        try {
            s = conn.get()
                    .prepareStatement("SELECT txOutChanges, transactions FROM store_undoableblocks WHERE hash = ?");
            // We skip the first 4 bytes because (on prodnet) the minimum target has 4 0-bytes
    ...
    public void setChainHead(StoredBlock chainHead) throws BlockStoreException {
        ...
        try {
            PreparedStatement s = conn.get()
                    .prepareStatement("UPDATE store_settings SET value = ? WHERE name = ?");
    ...
    private void removeUndoableBlocksWhereHeightIsLessThan(int height) throws BlockStoreException {
        try {
            PreparedStatement s = conn.get()
                    .prepareStatement("DELETE FROM store_undoableblocks WHERE height <= ?");
            s.setInt(1, height);
    ...
    public StoredTransactionOutput getTransactionOutput(Sha256Hash hash, long index) throws BlockStoreException {
        ...
        try {
            s = conn.get()
                    .prepareStatement("SELECT height, value, scriptBytes FROM store_openoutputs " +
                            "WHERE hash = ? AND ooindex = ?");
            s.setBytes(1, hash.getBytes());
    ...
    public void addUnspentTransactionOutput(StoredTransactionOutput out) throws BlockStoreException {
        ...
        try {
            s = conn.get().prepareStatement("INSERT INTO store_openoutputs (hash, ooindex, height, value, scriptBytes, toAddress, addressTargetable) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    public void removeUnspentTransactionOutput(StoredTransactionOutput out) throws BlockStoreException {
        maybeConnect();
        // This should only need one query
        ...
        try {
            PreparedStatement s = conn.get()
                    .prepareStatement("DELETE FROM store_openoutputs WHERE hash = ? AND ooindex = ?");
            s.setBytes(1, out.getHash().getBytes());
            // index is actually an unsigned int
            s.setInt(2, (int)out.getIndex());
            s.executeUpdate();
    ...
    public boolean hasUnspentOutputs(Sha256Hash hash, int numOutputs) throws BlockStoreException {
        ...
        try {
            s = conn.get()
                    .prepareStatement("SELECT COUNT(*) FROM store_openoutputs WHERE hash = ?");
            s.setBytes(1, hash.getBytes());
            ResultSet results = s.executeQuery();
            if (!results.next()) {
     ...
     * @throws org.casinocoin.store.BlockStoreException
     */
    public BigInteger calculateBalanceForAddress(Address address) throws BlockStoreException {
        ...
        try {
            s = conn.get().prepareStatement("select sum(('x'||lpad(substr(value::text, 3, 50),16,'0'))::bit(64)::bigint) "
                    + "from store_openoutputs where toaddress = ?");
