...
import org.casinocoin.crypto.KeyCrypter;
import org.casinocoin.crypto.KeyCrypterException;
import org.casinocoin.crypto.KeyCrypterScrypt;
import org.casinocoin.script.Script;
import org.casinocoin.script.ScriptBuilder;
import org.casinocoin.script.ScriptChunk;
...
import org.casinocoin.wallet.Protos.Wallet.EncryptionType;
import org.casinocoin.wallet.*;
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, org.casinocoin.wallet.WalletFiles.Listener)}
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        log.info("Received tx{} for {} {}: {} [{}] in block {}", sideChain ? " on a side chain" : "",
                Utils.bitcoinValueToFriendlyString(valueDifference), CoinDefinition.coinTicker,tx.getHashAsString(), relativityOffset,
    ...
    public static class SendRequest {
         ...
         *
         * <p>Once {@link Wallet#completeTx(Wallet.SendRequest)} is called, this is set to the
        ...
        /**
         * If not null, the {@link org.casinocoin.wallet.CoinSelector} to use instead of the wallets default. Coin selectors are
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            if (bestChangeOutput != null) {
                ...
                log.info("  with {} coins change", Utils.bitcoinValueToFriendlyString(bestChangeOutput.getValue()));
            }
            final BigInteger calculatedFee = totalInput.subtract(totalOutput);
            if (calculatedFee.signum() > 0) {
                log.info("  with a fee of {}", Utils.bitcoinValueToFriendlyString(calculatedFee));
            }
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, org.casinocoin.wallet.WalletFiles.Listener)}
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s %s (available: %s %s) in:%n",
                    Utils.bitcoinValueToPlainString(estimatedBalance), CoinDefinition.coinTicker, Utils.bitcoinValueToPlainString(availableBalance), CoinDefinition.coinTicker));
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(org.casinocoin.crypto.KeyCrypter,
     ...
     * {@link org.casinocoin.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all casinocoin controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(org.casinocoin.crypto.KeyCrypter,
     ...
     * you can use {@link org.casinocoin.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            if (selection3 == null && selection2 == null && selection1 == null) {
                ...
                log.warn("Insufficient value in wallet for send: needed {} more", Utils.bitcoinValueToFriendlyString(valueMissing));
